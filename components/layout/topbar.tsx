...
export function Topbar({ title, description }: { title: string; description: string }) {
  return (
    <header className="mb-6 flex flex-col gap-4 border-b border-[rgba(112,92,67,0.1)] pb-5 xl:flex-row xl:items-center xl:justify-between">
      <div>
        <h1 className="text-3xl font-semibold tracking-[-0.04em] text-[#2e251c]">{title}</h1>
        <p className="mt-2 max-w-3xl text-sm text-[#6b5a47]">{description}</p>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="flex items-center gap-3 rounded-full border border-[rgba(112,92,67,0.14)] bg-white px-4 py-2.5 text-sm text-[#6b5a47]">
          <Search className="h-4 w-4" />
          Search leads, campaigns, workspaces
        </div>
